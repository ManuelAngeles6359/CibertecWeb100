...
namespace Cibertec.Mocked
{
    public class UnitOfWorkMocked
    {
        ...
        private ICustomerRepository CustomerRepositoryMocked()
        {
            ...
            customerMocked.Setup(c => c.Insert(It.IsAny<Customer>())).Callback<Customer>((c) =>
               _customers.Add(c)).Returns<Customer>(c => c.Id);

            customerMocked.Setup(c => c.Update(It.IsAny<Customer>())).Callback<Customer>((c) =>
